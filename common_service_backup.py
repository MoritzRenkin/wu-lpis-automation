...
class Service(object):
    ...
    def start(self):
        ...
        try:
            ...
            if any("hide_console" == arg for arg in self.command_line_args()):
                print("in wrong if")
                self.process = subprocess.Popen(cmd, env=self.env, close_fds=platform.system() != 'Windows',
                                                stdin=PIPE, stdout=PIPE, stderr=PIPE, creationflags=0x08000000)

            else:
                self.process = subprocess.Popen(cmd, env=self.env, close_fds=platform.system() != 'Windows',
                                                stdout=self.log_file, stderr=self.log_file, stdin=PIPE,
                                                creationflags=0x08000000)

            # old before I changed it
            '''
